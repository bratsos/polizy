...
than one exists, `addMember`/`removeMember` need `as` to know which to write
(with exactly one group relation, `as` is inferred):
...
await authz.addMember({ member: alice, group: org,  as: "orgMember" });
```

Omitting `as` when it's ambiguous throws a `SchemaError`.
...
all hierarchy edges. When more than one exists, `setParent`/`removeParent` need
`as` (inferred when there's exactly one):

```typescript
relations: {
  folderParent: { type: "hierarchy" },  // document → folder
  orgParent: { type: "hierarchy" },     // folder → organization
}

await authz.setParent({ child: doc1, parent: folder, as: "folderParent" });
await authz.setParent({ child: folder, parent: org,   as: "orgParent" });
```

Each hierarchy relation propagates according to the same `hierarchyPropagation`
map (propagation is keyed by action, not by relation name).
...
> against the per-check `context`. See the conditions section in the parent SKILL and
> [polizy-patterns](../../polizy-patterns/SKILL.md).
