...
const schema = defineSchema({
  relations: {
    ...
    folderParent: { type: "hierarchy" }, // resource → folder
    orgParent: { type: "hierarchy" },    // folder/resource → org
  },
  actionToRelations: {
  ...
  hierarchyPropagation: {
    view: ["view_org"],  // viewing the org propagates to resources in it
  },
});

// Usage:
// - Each organization is a separate object
// - Resources point at their org:  setParent({ child: resource, parent: org, as: "orgParent" })
//   and at their folder:           setParent({ child: resource, parent: folder, as: "folderParent" })
// - Membership: addMember({ member: user, group: team, as: "member" })
//               addMember({ member: user, group: org,  as: "orgMember" })
// - org_member can view all resources in their org (via orgParent + hierarchyPropagation)
// - org_admin can manage all resources
// - Individual resource permissions for external sharing
// Note: `as` is REQUIRED here because the schema has >1 group and >1 hierarchy relation.
```
...
const schema = defineSchema({
  ...
  // Field-level permissions are critical here. Opt the "record" type in so ids
  // like "patient123#mental_health" split on the separator. Types NOT listed
  // never split (secure default).
  objectTypes: ["record"],
  fieldLevelObjects: ["record"],
  // fieldSeparator: "#" (default)
});

// Field-level usage (requires "record" in fieldLevelObjects above):
//   { type: "record", id: "patient123#mental_health" }
// A grant on the base record ("patient123") authorizes its fields via direct,
// group, and hierarchy paths; a grant on a field stays scoped to that field.
```
...
4. **Map roles to actions** - Which roles can do what? (every relation referenced must exist, or `defineSchema` throws)
5. **Identify groups** - Are there teams/departments? Multiple axes (team + org)? Name each `group` relation and pass `as` on writes.
6. **Identify hierarchies** - Are resources nested? Multiple axes (folder + org)? Name each `hierarchy` relation and pass `as` on writes.
7. **Define propagation** - Which actions inherit? (`hierarchyPropagation` actions must exist, or `defineSchema` throws)
8. **Decide field-level needs** - Any `type:id#field` ids? Add those types to `fieldLevelObjects` (opt-in; secure default is off).
9. **Plan conditions** - Time-boxed or attribute-gated grants? Use distinct relations for temporary vs standing (grants are idempotent per triple).
