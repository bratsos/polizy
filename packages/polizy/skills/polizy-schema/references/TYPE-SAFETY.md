...
export type Relation = keyof Schema["relations"];        // also: TypedRelation<Schema>
export type Action = keyof Schema["actionToRelations"];  // also: TypedAction<Schema>
// `as` on addMember/setParent is typed as TypedRelation<Schema>, so a wrong
// relation name is a compile error (and a SchemaError at runtime if it's the
// wrong kind).
