...
All examples build one shared `authz` instance per process and import it where
needed. They use the Prisma adapter via `PrismaStorageAdapter` (a factory — call
it, no `new`) from the `polizy/prisma-storage` subpath; see
[PRISMA-SETUP.md](PRISMA-SETUP.md) for the required `PolizyTuple` model and
migration. Swap in `new InMemoryStorageAdapter()` from `"polizy"` for tests.

> **Typing actions:** `check`'s `canThey` is typed to your schema's actions
> (`TypedAction`). The middleware/helpers below take `action: string` for
> brevity; for full type-safety, type the parameter as `TypedAction<typeof schema>`
> (or a string-literal union of your actions) so typos are caught at compile time.
...
## Tips for these integrations

- **`checkOrThrow` trims the boilerplate.** The handlers above all follow
  "`check` then throw on `false`". You can collapse that to a single call:
  `await authz.checkOrThrow({ who, canThey, onWhat })` throws
  `NotAuthorizedError` when denied — catch it in an error filter / middleware
  and map it to a 403.
- **Batch with `checkMany`** when a handler renders a list (e.g. which rows the
  user may edit): `const flags = await authz.checkMany(rows.map(r => ({ who, canThey: "edit", onWhat: { type: "document", id: r.id } })))` avoids an N+1
  loop of checks.
- **Depth errors are real now.** Past `defaultCheckDepth` (20), `check` throws
  `MaxDepthExceededError` rather than returning `false`. Either let your error
  handler surface it, or construct `AuthSystem` with `maxDepthBehavior: "deny"`.

For richer authorization scenarios (teams, folder inheritance, temporary access,
field-level permissions), see **[polizy-patterns](../../polizy-patterns/SKILL.md)**.
