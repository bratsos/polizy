...
## Multiple Group Relations (org vs. team)

If you model both org membership and team membership as **group** relations,
that's two group relations — so `addMember`/`removeMember` need `as`:

```typescript
const schema = defineSchema({
  relations: {
    orgMember: { type: "group" },   // user → org
    teamMember: { type: "group" },  // user → team, team → org
    // ...resource roles
  },
  // ...actionToRelations
});
...
await authz.addMember({ member: user, group: team, as: "teamMember" });
await authz.addMember({ member: team, group: org,  as: "orgMember" });
```

With a single group relation (as in the schemas above), omit `as`.

## Bulk Provisioning with allowMany

When seeding an org (owner + member + a starter resource), `allowMany` issues all
the idempotent grants in one call instead of a chain of `allow()`s:

```typescript
await authz.allowMany([
  { who: owner, toBe: "org_owner",  onWhat: org },
  { who: owner, toBe: "org_member", onWhat: org },
  { who: owner, toBe: "owner",      onWhat: { type: "document", id: "welcome" } },
]);
```

## Auditing Who Can Access a Resource

For a share dialog or a security audit, list every subject that can perform an
action on a resource — including users reachable via org membership and hierarchy:

```typescript
const viewers = await authz.listSubjects({
  canThey: "view",
