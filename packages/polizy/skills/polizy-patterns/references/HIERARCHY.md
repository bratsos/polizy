...
> **One hierarchy relation vs. many.** With a single `hierarchy` relation,
> `setParent`/`removeParent` infer it. If you declare more than one (e.g.
> `parent` and `folderParent`), every parent write/remove must pass
> `as: "<relation>"` or it throws a `SchemaError`. See
> [Multiple Hierarchy Relations](#multiple-hierarchy-relations).
...
const schema = defineSchema({
  relations: {
    parent: { type: "hierarchy" },        // generic containment
    folderParent: { type: "hierarchy" },  // filesystem-style
    viewer: { type: "direct" },
  },
...
await authz.setParent({
  child: { type: "document", id: "doc1" },
  parent: { type: "folder", id: "folder1" },
  as: "folderParent"   // required when >1 hierarchy relation exists
});
...
When a type is in `fieldLevelObjects`, base-object access propagates to its fields
**through the hierarchy** too. A folder viewer reaches `doc#field` of documents in
that folder (folder → doc base → field):

```typescript
// schema has fieldLevelObjects: ["document"], a hierarchy relation, and
// hierarchyPropagation: { view: ["view"] }
await authz.setParent({ child: { type: "document", id: "doc1" },
                        parent: { type: "folder", id: "folder1" } });
await authz.allow({ who: { type: "user", id: "alice" }, toBe: "viewer",
                    onWhat: { type: "folder", id: "folder1" } });

await authz.check({ who: { type: "user", id: "alice" }, canThey: "view",
                    onWhat: { type: "document", id: "doc1#summary" } }); // true
```
