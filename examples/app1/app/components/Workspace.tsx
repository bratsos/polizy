...
const btn =
  "inline-flex items-center gap-1 rounded-md border border-zinc-700 bg-zinc-800 px-2.5 py-1 text-xs font-medium text-zinc-200 transition-colors hover:bg-zinc-700 disabled:cursor-not-allowed disabled:opacity-40";
const input =
  "w-full rounded-md border border-zinc-700 bg-zinc-900 px-2.5 py-1.5 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-indigo-500 focus:outline-none";
...
export default function Workspace({
  ...
  const Row = ({ e, nested }: { e: Entity; nested?: boolean }) => (
    <Link
      ...
      title={
        e.accessible ? undefined : `No access to ${e.name} as the current user`
      }
      className={`group flex items-center justify-between rounded-md px-2 py-1.5 text-sm transition-colors ${
        nested ? "ml-4" : ""
      } ${openKey === e.key ? "bg-indigo-500/15 text-indigo-200" : "hover:bg-zinc-800/70"} ${
        e.accessible ? "text-zinc-200" : "text-zinc-500"
      }`}
    >
      ...
      {e.accessible && e.actions.length > 0 && (
        <span className="ml-2 hidden shrink-0 gap-1 group-hover:flex group-focus-within:flex">
          {e.actions.slice(0, 3).map((a) => (
            <span
              key={a}
              className="rounded bg-zinc-800 px-1 text-[10px] text-zinc-400"
            >
              {a}
            </span>
          ))}
  ...
  return (
    <>
      <aside className="border-r border-zinc-800 bg-zinc-950 p-3 lg:min-h-[calc(100vh-57px)]">
        <nav className="space-y-0.5">
          ...
          {teams.length > 0 && (
            <div className="pt-2 text-[10px] uppercase tracking-wide text-zinc-600">
              Teams
            </div>
          )}
          {teams.map((t) => (
  ...
  return (
    <div className="mx-auto mt-20 max-w-md text-center text-zinc-500">
      ...
      <p className="mt-3 text-sm">
        ...
        <span className="font-medium capitalize text-zinc-300">{persona}</span>.
    ...
    <div className="mx-auto max-w-3xl">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 rounded-md bg-indigo-500 px-3 py-1.5 text-sm font-medium text-white hover:bg-indigo-400"
        >
          🔍 Inspect access
        </Link>
      </div>
      ...
      <ul className="space-y-1 text-sm">
        {(opened.members ?? []).map((m) => (
          <li key={m.id} className="flex items-center justify-between">
            <span className="capitalize text-zinc-300">👤 {m.name}</span>
        ...
        {(opened.members ?? []).length === 0 && (
          <li className="text-zinc-600">No members.</li>
        )}
      </ul>
      {allow("manage_members") && (
    ...
    <div className="mt-4 flex flex-wrap items-center gap-2">
      ...
      {allow("delete") && (
        ...
        >
          ...
          <input type="hidden" name="resourceKey" value={opened.key} />
          <button
            className={`${btn} border-rose-800 text-rose-300 hover:bg-rose-900/40`}
            type="submit"
          >
            🗑 Delete
          </button>
...
function ShareControls({
  ...
  return (
    <details className="w-full rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
